<template lang="pug">
XPArticlesCategory(:title="title")
</template>

<script>
import XPArticlesCategory from '~/components/pages/Articles/Category'

export default {
  components: { XPArticlesCategory },
  computed: {
    title() {
  ...
  mounted() {
    this.$appClient.segment.trackPageView({ page: 'Articles' })
  },
  head() {
    return {
      title: `${this.title} Articles | ActivePlace`,
      link: [{ rel: 'canonical', href: `${this.baseURL}${this.$route?.fullPath}` }],
      meta: [
        {
          hid: 'twitter:title',
          name: 'twitter:title',
          content: `${this.title} Articles | ActivePlace`,
        },
        {
